...
generate_mesh_DLPFC <- function(X, V, Clustering){
  ...
  polygons <- concaveman(as.matrix(out_boundary), concavity = 1, length_threshold = 12.4)
  ...
  simplified_polygon <- gSimplify(spatial_polygons, tol = 0.05)@polygons[[1]]@Polygons[[1]]@coords
  ...
  mesh <- refine.mesh.2D(mesh, minimum_angle = 30, maximum_area = 60)
